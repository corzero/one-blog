...
class YiBoShi {
  ...
  async getUnfinishList() {
    ...
    console.log(`未完成任务列表：${list.length}`)
    // 已完成
    ...
    if (isfinished.length < this.score) {
      ...
      console.log('当前用户25分满足，无需刷分，任务结束')
    }
  }
  // 获取视频，答题列表
  async getRealUrlList() {
    for (let i = 0; i < this.unfinish.length; i++) {
      ...
      // console.log('当前值', this.unfinish[i])
      // console.log('返回参数', res.data.data.data[0].id)
      const video = {
    ...
    console.log('------------视频列表，答题列表获取成功......，准备开始。\n\n')
  }
  // 刷时间
  async setVideoTime() {
    if (this.videoList.length < 1) {
      console.log('视频列表为空。时间任务结束.....\n\n')
    } else {
      ...
      console.log('视频观看时间增加成功, 开始答题')
    }
  }
  // 答题
  async awanser() {
    if (this.questionList.length < 1) {
      console.log('题目列表为空。答题任务结束')
    } else {
      console.log(`未答题${this.questionList.length}个题目，准备答题`)
  ...
  async applyCredit() {
    ...
    console.log(`未申请学分${unCredit.length}个`)
